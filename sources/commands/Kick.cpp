...
/* Functions */
	void	Kick::exec_cmd(User &user ) {
		if (_channel.size() == 1)
		{
			...
			for (std::vector<std::string>::const_iterator it = _nickname.begin(); it != _nickname.end(); ++it)
			{
				...
				std::string ret = ":" + user.get_nickname() + " KICK " + (*chan)->get_name() + " " + parting_user->get_nickname() + "\r\n"; // Add Comment Here i think
				Proxy_queue::Write * msg = new Proxy_queue::Write(parting_user->get_socket()->get_fd(),ret.c_str());
				_server->get_event_list().push_back(msg);
				(*chan)->transmit_message( " KICK " + (*chan)->get_name(), parting_user); // Maybe PART instead of KICK here for the client to understand what to do
				if ((*chan)->get_user_list().size() == 1){
					_server->del_channel(*(*chan));
					break;
				}
				parting_user->leave_channel(*it);
			}
		}
		else {
			for (std::vector<std::pair<User *,Channel *> >::iterator it = _chan_user_pair.begin(); it != _chan_user_pair.end();it++)
			{
				...
				std::string ret = ":" + user.get_nickname() + " KICK " + it->second->get_name() + " " + parting_user->get_nickname() + "\r\n"; // Add Comment Here i think
				Proxy_queue::Write * msg = new Proxy_queue::Write(parting_user->get_socket()->get_fd(),ret.c_str());
				_server->get_event_list().push_back(msg);
				it->second->transmit_message( " KICK " + it->second->get_name(), parting_user); // Maybe PART instead of KICK here for the client to understand what to do
				if (it->second->get_user_list().size() == 1){
	...
	bool Kick::is_valid_args(User const &user) {
		...
		std::string kick_message = "You have beed kicked from the channel";
		if(_args.size() < 3) {
			throw error(user.get_nickname() + "Not engouth PARAMS", ERR_NEEDMOREPARAMS);
		}
		if (_args.size() == 4) {
			kick_message = _args[3];
		}
		split_arguments();
		...
		for (std::vector<std::string>::iterator it = _channel.begin(); it != _channel.end(); ++it){
			...
			if (_channel.size() == 1)
			{
				for (std::vector<std::string>::iterator it2 = _nickname.begin(); it != _nickname.end(); ++it2) {
				...
				vec_user tmp = (*chan)->get_user_list();
				for (std::vector<User *>::const_iterator userit = tmp.begin(); userit != tmp.end(); ++it) {
