...
/* Operators */
/* Functions */
	void User_cmd::exec_cmd(command const &cmd, User const &user) const {
	...
	void User_cmd::is_valid_args(Server const *Server, User const &user) const {
		...
		for (std::string::iterator it = _args[1].begin(); it != _args[1].end(); ++it;) {
			if ((*it) == %x01-09 or (*it) == %x0B-0C  or (*it) == %x0E-1F or (*it) == %x21-3F or (*it) == %x41-FF)
				throw error("Invalid username", 0);
		for (string::iterator it = _args[4].begin(); it != _args[4].end(); ++it;) {
